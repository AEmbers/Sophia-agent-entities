  ...
  ctx.effect(() => {
    ...
    dispose.push(ctx.commands.register({
      ...
      handler(invocation: CommandInvocation): CommandResult {
        ...
        if (parsed.profile !== undefined && !Object.keys(getProfiles()).some(key => key.trim() === parsed.profile)) return { kind: 'error', text: `unknown Qintianjian profile "${parsed.profile}"` }
    ...
    for (const profileName of Object.keys(getProfiles())) {
      ...
      dispose.push(ctx.commands.register({
        ...
        handler(invocation: CommandInvocation): CommandResult {
          ...
          if (profile === undefined) return { kind: 'error', text: `Qintianjian profile command "/${commandName}" is unavailable` }
