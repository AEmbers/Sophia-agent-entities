export const zh = {
  ...
  workspaces: '工作区',
  // ── 左栏（侧边栏）新增的三条 ──
  // ① 工作区选择器的可访问名（上游 `TeamWorkspaceSelector` 的 `aria-label`）。
  workspaceSelectorWithValue: '工作区：{title}',
  // ② 成员生命周期里的「停用」。索菲亚只有可逆的 suspend/resume，
  //    上游的 `archiveAgent`（归档，墓碑）与它不是同一件事 ⇒ 另给一个词。
  suspendAgent: '停用',
  // ③ 两个「控件保留、能力没有」的如实说明（见 `TeamChannelsPanel` /
  //    `TeamAgentsPanel` 的移植点注释）。这两句话本身就是那两处的降级理由，
  //    不是装饰文案。
  sidebarChannelDescriptionUnsupported: '索菲亚宿主没有频道描述字段（路由只接受名称），本框仅用于对照上游，不会发送',
  sidebarAddAgentDescriptionUnsupported: '索菲亚宿主没有成员描述字段（路由只接受职位名），本框仅用于对照上游，不会发送',
  sidebarModelCatalogUnavailable: '索菲亚没有接宿主模型目录，成员模型跟随全局默认',
  // ④ 独立评审补的两条（都是「控件保留、能力没有 ⇒ 必须明说」）。
  //    建频道的「初始成员」多选：索菲亚的建频道路由只收 `{teamId, title}`，
  //    这个选择**发不出去** ⇒ 恒禁用 + 这一行（否则是用户输入静默丢失）。
  sidebarInitialMembersUnsupported: '索菲亚的建频道接口只接收名称，不支持指定初始成员；此选择器不会随表单提交',
  //    Agents 的「名称」框：它填的值会当成**职位名**发给宿主，而职位名是闭集
  //    （`src/naming.ts` 的 `POSITIONS`）⇒ 必须先把规则说清楚，别让用户靠撞错才发现。
  sidebarPositionHint: '此处填的是职位名（不是随意起的名字）；宿主只接受其职位名册里的取值，填错时宿主的回执会列出全部合法职位名',
  channels: '频道',
  agents: 'Agents',
  members: '成员',
  ...
  // 「换模」进左栏 Agents 行菜单（2026-09-24 从旧团队卡搬进来）。
  // 语义：把该成员当前模型重新提交宿主（seats.tsx 同款）；title 是给按钮的说明。
  switchModel: '换模',
  switchModelAgentHint: '把该成员当前的模型目标重新提交给宿主（跟随默认的成员没有可提交目标）',
} as const
...
export const en = {
  ...
  // ── 左栏（侧边栏）新增的三条（与 `zh` 同键，见那一侧的说明）──
  workspaceSelectorWithValue: 'Workspace: {title}',
  suspendAgent: 'Suspend',
  sidebarChannelDescriptionUnsupported: 'The Sophia host has no Channel description field (the route takes a name only); this control exists to mirror upstream and is not sent',
  sidebarAddAgentDescriptionUnsupported: 'The Sophia host has no Member description field (the route takes a position only); this control exists to mirror upstream and is not sent',
  sidebarModelCatalogUnavailable: 'Sophia does not mount the Host model catalog; a Member follows the global default',
  sidebarInitialMembersUnsupported: 'Sophia\u2019s create-channel endpoint takes a name only and cannot specify initial Members; this picker is not submitted with the form',
  sidebarPositionHint: 'This field is a Position name, not a free-form name; the Host accepts only values from its Position roster, and its reply lists every valid Position when the value is rejected',
  channels: 'Channels',
  agents: 'Agents',
  members: 'Members',
  ...
  switchModelAgentHint: 'Resubmit this member\u2019s current model target to the Host (members following the default have no target)',
} as const
