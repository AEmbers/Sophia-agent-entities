 ...
 *
 * ⚠ 索菲亚移植点（**只改取用方式，其余逐字照抄**）——与 `sidebar-sections.ts` 同源：
 * 1. React 的 `useCallback` / `useSyncExternalStore` 走 `react-runtime.ts` 的
 *    `hooks()` 惰性取用（顶层值导入会撞红「产物里顶层模块加载调用 ≤ 2」的断言）。
 * 2. `WorkspaceId` 从 `./ids.ts` 取（索菲亚没有上游那个包）。
 */
