...
// —— 顶层**值导入**，会多出一条顶层模块加载调用，撞红「`require(` ≤ 2」的既有断言
// （实测 `lib/client.js` 当前恰好 2 次：惰性 react + 惰性 primitives 代理）。
// 索菲亚的既有手法是**惰性取**（详见 `../../react-runtime.ts` 的文件头对照表）：
// hook 在组件渲染期经 `hooks()` 取；primitive 组件经 `../bridge.ts` 的惰性代理取。
// `import type { ChangeEvent, … } from 'react'` 原本就是 `type`（会被完全擦除），故保留。
import type { ChangeEvent, ClipboardEvent, KeyboardEvent } from 'react'
import { hooks } from '../../react-runtime.ts'
import { primitives } from '../bridge.ts'
import type { AgentTeamClientMemberStatus, AgentTeamMemberId } from './agent-team-types.ts'
...
// ⚠ 索菲亚移植点：上游这里是 ./attachment-preview.ts（整条附件字节→URL 链）。
// 索菲亚没有附件能力 ⇒ 该文件整条剥掉，只把仍在用的 ormatByteSize`r
// 并入 	eam-formatters.ts（理由见那个文件的头部差异表）。
import { formatByteSize } from './team-formatters.ts'
...
export function TeamComposer({ members, followerMemberIds, drafts, draftKey, pending, confirmation, error, onEdit, onSubmit, placeholder, pendingFiles, onFilesChange, asTask, onAsTaskChange, t }: {
  ...
  // 索菲亚移植点：hook 经 hooks() **惰性取**（见顶部 import 处的说明）。
  const { useEffect, useLayoutEffect, useRef, useState, useSyncExternalStore } = hooks()
  ...
  const menuMaxHeight = primitives.useAnchoredMaxHeight(menuRef, 320, menuOpen ? draft : null)

  primitives.useDismissOnOutsidePointer(rootRef, menuOpen, open => { if (!open) setMention(undefined) })
      ...
      <div className={css.toolbar}>
        {onFilesChange !== undefined && (
          <>
            ...
            <primitives.Tooltip label={t('attachFiles')} delayMs={500}>
              <button type="button" className={css.attachButton} aria-label={t('attachFiles')}
                ...
                <primitives.IconPaperclipOutline16 size={14} />
              </button>
            </primitives.Tooltip>
          </>
        )}
        {onAsTaskChange !== undefined && (
          ...
          >
            <primitives.IconChecklistOutline14 size={14} />
        ...
        >
          <primitives.IconSendOutline16 size={16} />
        </button>
      </div>
    </div>
