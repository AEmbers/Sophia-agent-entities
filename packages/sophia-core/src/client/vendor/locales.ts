...
export const zh = {
  ...
  // ⚠ 索菲亚新增（改动之二的禁用态）：宿主半没有「团队级中止」路由
  //   ⇒ 停止键的说明文案。`*Unavailable` 是给按钮 title 的一句话，
  //   `*UnavailableDetail` 是弹窗里给用户看的原因（写清"不会发请求"）。
  //   键必须同时加进 `en`（`en: Record<LocaleKey, string>` 会把漏键变成 TS2739，
  //   另有运行期断言 `tests/client/view-model.spec.ts:99-101` 比对两侧键集合）。
  'team.stopUnavailable': '停止团队（当前版本不可用：宿主无团队级中止路由）',
  'team.stopUnavailableDetail': '当前版本不能中止整个团队：索菲亚宿主半尚未提供团队级中止路由，因此这里不会发出任何请求。宿主补上该路由后即可恢复。',
  'team.discarded': '已放弃',
...
export const en = {
  ...
  'team.stopUnavailable': 'Stop team (unavailable: this build has no host halt route)',
  'team.stopUnavailableDetail': 'This build cannot stop a whole team: the Sophia host half provides no team-level halt route yet, so no request is sent. It becomes available once the host provides one.',
