...
import { PENDING_STATES } from './sophia-approval-card-definition.ts'
 ...
 */
export type ApprovalSettlement = { readonly state: string } | 'gone'
...
export function settlementOf(
  ...
  live: ApprovalLiveState,
): ApprovalSettlement | undefined {
  if (resultState !== undefined) {
    return PENDING_STATES.has(resultState) ? undefined : { state: resultState }
  }
  if (live === 'absent') return 'gone'
  if (live !== undefined && !PENDING_STATES.has(live.state)) return { state: live.state }
  return undefined
}
...
export type ApprovalSettlementLabel =
  ...
  | { readonly key: 'approval.settled.other'; readonly state: string }

/** Map a settlement onto its wording; an unknown state falls back to itself. */
export function settlementLabelOf(settlement: ApprovalSettlement): ApprovalSettlementLabel {
