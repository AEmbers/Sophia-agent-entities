...
function readInstalledBundleVersion() {
    try {
...
export function normalizeHumanName(raw) {
 ...
 */
export function assertValidHumanName(raw) {
    const name = normalizeHumanName(raw);
    if (name === '')
        throw new Error('human name must not be empty');
    return name;
}
/**
 ...
 */
export function parseLegacyHumanProfile(yamlText) {
    let document;
    try {
    ...
    if (typeof document !== 'object' || document === null)
        return undefined;
    const section = document[LEGACY_HUMAN_PROFILE_SECTION];
    ...
    const avatarRef = typeof fields.avatarRef === 'string' && fields.avatarRef.trim() !== ''
        ...
        : undefined;
    if (name === undefined && avatarRef === undefined)
        return undefined;
 ...
 * already hold.
 */
export function planLegacyAdoption(current, legacy) {
    if (current.name !== HUMAN_PROFILE_DEFAULT_NAME || current.avatarRef !== undefined)
        return undefined;
