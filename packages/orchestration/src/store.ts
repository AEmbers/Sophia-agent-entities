...
export function teamLockQueueKeys(): string[] {
  return [...locks.keys()]
}
...
function isRetryableRenameError(error: unknown): boolean {
  return error instanceof Error
    ...
    && ['EPERM', 'EACCES', 'EBUSY'].includes((error as NodeJS.ErrnoException).code as string)
}
...
export function approvalsRootOf(workspace: string): string {
  return join(workspace, '.sophia-entities', 'approvals')
}

export function approvalFileOf(root: string, requestId: string): string {
  return join(root, `${requestId}.json`)
}

/** Directory that holds the request files; created lazily on first save. */
async function ensureApprovalsRoot(root: string): Promise<void> {
  await mkdir(root, { recursive: true })
}

function parseRequest(content: string): ApprovalRequest {
...
export async function readApproval(root: string, requestId: string): Promise<ApprovalRequest | undefined> {
  try {
...
export async function listApprovals(root: string): Promise<ApprovalRequest[]> {
  let files: string[]
  try {
...
export async function writeApproval(root: string, request: ApprovalRequest): Promise<void> {
  await ensureApprovalsRoot(root)
  await atomicWriteText(
    approvalFileOf(root, request.id),
    `${JSON.stringify(request, null, 2)}\n`,
  )
}

/** Remove a request file (ENOENT is fine). */
export async function deleteApproval(root: string, requestId: string): Promise<void> {
  try {
...
export function goalDigest(goal: string): string {
