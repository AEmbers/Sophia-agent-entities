...
import { approvalsRootOf, goalDigest, listApprovals, readApproval, withLock, writeApproval, } from "./store.js";
...
export class ApprovalRouter {
    ...
    materializeHook;
    constructor(workspace, host, options = {}) {
    ...
    async findDuplicate(goal, plan) {
        ...
        return existing.find((r) => !isTerminalState(r.state) && r.goal === goal
    ...
    async setMode(requestId, mode) {
        return this.store.withLock(`request:${requestId}`, async () => {
            ...
            const updated = { ...request, mode, updatedAt: this.getNow() };
    ...
    async review(requestId, verdict) {
        return this.store.withLock(`request:${requestId}`, async () => {
            const request = await this.requireRequest(requestId);
            ...
            switch (verdict.decision) {
                ...
                case 'reject': {
                    const rejected = { ...base, state: 'rejected', expiresAt: undefined };
    ...
    async approve(requestId, verdict) {
        return this.store.withLock(`request:${requestId}`, async () => {
            const request = await this.requireRequest(requestId);
        ...
        for (const request of await this.store.listApprovals(this.root())) {
            ...
            const outcome = await this.store.withLock(lockKey, async () => {
                ...
                if (!latest || isTerminalState(latest.state))
                    return undefined;
                if (latest.expiresAt && latest.expiresAt > this.getNow())
                    return undefined;
     ...
     */
    async materializeOrHalt(request, mode, successState) {
        ...
        try {
            ...
            throw new MaterializeError(`materialization failed: ${error.message}`, { ...request, updatedAt: this.getNow() });
        }
    }
}
