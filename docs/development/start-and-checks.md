...
- `check:artifact` reads back what `npm pack` would actually ship and refuses an artifact that would ship broken: stray `.ts`/`.tsx` sources, a missing `cordis.patch.yml`, a runtime relative import whose target is not inside the tarball, or a runtime asset directory — `fileURLToPath(new URL('../assets/…', import.meta.url))` — with no shipped file under it. Those last two are invisible to every local check, because the working tree has every file. Run it after `build` and before publishing.
