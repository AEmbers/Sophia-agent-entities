...
- Dead artwork is gone: seven files nothing referenced (the retired whale lead, three activity frames no state maps to, a design mock and a hero image) left the repository and the pack, and `packages/dag-team/tests/runtime-assets.spec.ts` now pins the whale directory to the eleven images the client actually asks for, so unused art cannot creep back in.
- A decision sticks: the proposal card now reads the verdict the Host returns for the action (`state`, `mode`) and reconciles once against the live pending queue, instead of trusting the `pending_*` state frozen into the transcript. Returning or approving a proposal makes the card say so and retire its buttons, rather than answering every further click with "is not awaiting owner decision".
- Approving a proposal actually builds the team. The orchestration facade calls the DAG backend with *itself*, and the facade carries no team state — so `captain` and `stateRoot`, which materialization cannot proceed without, were never handed over, and every approval of a `dag` request died as an HTTP 500 the card could only echo. The approval plane now keeps that state and reads it fresh per call (`hostContext`), and the route binds the deciding session as the captain just before the action runs, so the materialized team lands in the decider's workspace.
- The Web surfaces carry the organisation's own name: the activity panel, the sidebar tab, the collapsed badge, the proposal card and the `/agent-teams` command's own text all say `钦天监` instead of the borrowed "AgentTeams" (English surfaces use `Qintianjian`). Model-facing prompts, the `agent_teams_*` tool names and the durable mailbox receipt marker keep the Host's protocol wording.
